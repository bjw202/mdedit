...
### Added
- **대용량 마크다운 파일 편집 동결 해소 (SPEC-IMG-LOAD-002 Group A)**: 여러 이미지를 base64로 임베드한 마크다운 문서를 열면 에디터가 **동결**되어 입력에 반응하지 않던 문제를 수정했습니다 — 001 Group B가 "5MB 초과 파일은 빈 화면 대신 건너뜀 placeholder" 안전망을 깔아놓은 뒤에도, **5MB 이하**에서 거대 base64 라인이 하나라도 있으면 편집·렌더가 걍 멎었던 것이 이어지는 고통이었습니다. 원인은 이미지 위젯 데코레이션 계산이 `docChanged`마다 `view.state.doc.toString()`으로 **문서 전체를 복사**하고 있었기 때문입니다(`src/components/editor/extensions/image-widget.ts:155-170`). 마크다운 한 라인이 base64 data URI로 수십 MB에 달하면 이 동기 복사 비용이 매 입력마다 발생해 메인 스레드를 잡아먰습니다. 위젯 데코레이션 계산 범위를 문서 전체 대신 `view.visibleRanges`로 한정하여 전체 복사 비용을 제거했습니다(REQ-A-001 — 이것이 동결의 직접 원인). 뷰포트가 바뀔 때마다 위젯을 재계산하도록 `viewportChanged` 트리거도 추가했습니다(REQ-A-002). 본 수정만으로 사용자가 보고한 동결이 해소됨을 실측으로 확인했습니다 — **linchpin PT-A1-006b PASS** (4MB `.md` + 거대 base64 라인, 뷰포트 안으로 스크롤 후 타이핑 → 첫 paint 941ms, 예산 5s 이내 — Phase 2의 스트리밍 읽기·Web Worker 이관은 불필요로 판명되어 이월).
- **거대 base64 라인 자동 폴딩 (SPEC-IMG-LOAD-002 Group A)**: 단일 라인 길이가 `LINE_FOLD_THRESHOLD`(1MB)를 초과하면 자동으로 fold하여 시각적으로 축소합니다(REQ-A-003/004/005) — 폴드된 라인은 축소 표시로 나타나며 클릭하여 펼치거나 다시 접을 수 있습니다. 폴딩은 디스플레이 비용만 줄이며 Lezer 토크나이제이션 비용은 그대로입니다(독립 감사 확증 — `@codemirror/language/dist/index.js:612-625`), 동결 해소는 위의 뷰포트 바운딩(REQ-A-001)이 담당합니다. 삽입 직후(`insertImageMarkdown`)에도 임계값 초과 라인을 바로 fold합니다. CodeMirror `StateField<Decoration.fold>` 대신 `foldEffect`를 사용했습니다 — 사용자 토글이 정상 동작하기 위해서입니다.
- **임계값 정책 5MB 단일 블록 → 3계층 재정의 (SPEC-IMG-LOAD-002 Group D)**: 기존 `FILE_SIZE_THRESHOLD = 5MB` 단일 상수 한계를 `SOFT_THRESHOLD`(30MB) / `HARD_CEILING`(100MB) / `LINE_FOLD_THRESHOLD`(1MB) 3계층으로 재정의했습니다. (1) `.md`/`.markdown` 파일은 이제 **100MB까지 편집 가능** — 이전에는 5MB만 넘어도 `UnsupportedFileViewer` placeholder로 열리 편집이 불가했습니다(편집 상한이 20배 상향). 30MB 초과 시 라인 폴딩이 자동 활성화됩니다. (2) 100MB 초과 파일은 기존대로 `UnsupportedFileViewer`로 라우팅·편집 잠금(001 Group B 동작과 정합). (3) 기존 `FILE_SIZE_THRESHOLD`(5MB) 상수는 **삭제하지 않고 deprecated alias로 보존** — `SPEC-PREVIEW-007` 회귀 방지용이며 SvgFileViewer 가드도 그대로입니다. (4) 래스터 이미지 뷰어(`.png`/`.jpg`/...)와 SVG 뷰어는 자체 임계값 처리를 그대로 사용합니다(SPEC-PREVIEW-008 무간섭). 라우팅 경계값이 5MB → 100MB로 이동하면서, 기존 5MB를 단언하던 3개의 brownfield 테스트(`src/test/SPEC-PREVIEW-007.test.tsx` 시나리오 E, `src/test/specImgLoadBFileSystem.test.ts` UT-B4a/c, `e2e/spec-img-load-001.spec.ts` PT-B4 seed)를 100MB 기준으로 보정했습니다 — 의도(한계 초과 = too-large)를 보존한 숫자 갱신입니다. 검증: tsc·eslint 클린, vitest 1521/1521(+47), cargo 336/336(Group D는 프론트엔드 전용), cargo clippy 클린, Playwright 60/60(001 회귀 PT-A1 1.2s·PT-B4 632ms 포함; 002 PT-A1-006 1.0s·linchpin PT-A1-006b 941ms).

### Fixed
- **대용량 마크다운 파일 재오픈 안전망 (SPEC-IMG-LOAD-001 Group B)**: 여러 이미지를 base64로 붙여넣은 마크다운 파일은 용량이 크게 불어나는데, 이걸 다시 열면 5MB 초과 시 에디터가 **빈 화면**으로 열리거나(에러 배너 없음), 반대로 5MB 가드가 우회되면 **메인 스레드가 동결**되는 결함이 있었습니다. 5개의 하위 결함을 묶어 수정했습니다. (1) `getFileViewType` 라우팅 순서 — `.md`/`.markdown` 확장자 분기가 `too-large` 분기보다 먼저 평가되어, 5MB 초과 `.md` 파일이 빈 에디터 + 빈 프리뷰로 열렸습니다. `too-large` 분기를 `.md`/`.markdown`에 한해 먼저 평가하도록 재배치했습니다(다른 확장자의 분기 순서는 SPEC-PREVIEW-008 래스터/SVG 보호를 위해 현행 유지). (2) 접힌 폴더 내 파일 가드 우회 — 탐색기에서 접히지 않은 폴더 안의 파일은 트리에 size가 없어 5MB 가드가 스킵되고 전체가 로드되었습니다. 신규 `read_file_size` IPC로 size를 사전 조회해서 가드를 적용합니다(펼쳐진 폴더는 기존 size를 그대로 써서 IPC 호출 없이 빠른 경로 유지). (3) 와쳐 reload 우회 — 파일 워쳐가 수정 이벤트를 받을 때 크기 가드를 거치지 않고 `readFile`을 직접 호출하던 걸 `openFile` 경유로 변경했습니다. (4) 비원자 쓰기 — `write_file`이 임시 파일 없이 `std::fs::write` 한 번으로 쓰고 있어, 크래시·전원 손실 시 파일이 잘리거나 손상될 수 있었습니다. 임시 파일 → `fsync` → `rename` 패턴으로 원자화했습니다(POSIX은 cargo 테스트로 자동 검증, Windows는 `ReplaceFile` 동등 패턴으로 최선 노리되 자동화 CI는 범위 밖 — 수동 스모크로 확인 필요). (5) 같은 이유로 영향 받던 `image_ops.rs` 테스트 코드의 기존 clippy 에러 4건도 함께 정리했습니다(`-D warnings` 게이트 통과용, 동작 변화 없음). 임계값 숫자(`FILE_SIZE_THRESHOLD=5MB`)는 바꾸지 않았습니다. **주의**: base64 임베드로 생기는 거대한 단일 라인을 CodeMirror가 라인 단위로 토크나이즈하는 비용은 이 안전망으로 막히지 않습니다 — 뷰포트 렌더링·라인 폴딩·스트리밍 읽기·Web Worker 이관은 후속 SPEC-IMG-LOAD-002로 이월되었습니다. 본 PR은 "빈 화면·UI 동결"이라는 가장 가시적인 증상의 안전망이지, 대용량 파일의 완전한 부드러움을 보장하지는 않습니다. 검증: tsc·eslint 클린, vitest 1474/1474, cargo clippy(`-D warnings`) 클린, cargo test 336/336, Playwright PT-A1/PT-B4 2/2.
- **이미지 삽입 호출부가 모드를 인지하도록 수정 (SPEC-IMG-LOAD-001 Group A)**: 문서를 아직 저장하지 않은 상태에서 툴바의 이미지 버튼이나 `Cmd/Ctrl+Shift+I`를 누르면, 이미지 피커가 열리기 전에 **"다른 이름으로 저장" 다이얼로그**가 먼저 떴습니다 — 기본 모드인 **Inline**(base64 data URI로 문서에 바로 임베드)에서는 저장 경로가 아예 필요 없음에도 불구하고요. 원인은 두 삽입 진입점(`AppLayout.tsx`의 툴바 케이스, `MarkdownEditor.tsx`의 `Cmd+Shift+I`)이 `imageInsertMode`를 읽지 않고 `currentFilePath === null`이면 무조건 `saveFileAs`를 먼저 돌리던 게이트를 타고 있었기 때문입니다. 이전 SPEC-IMG-MODE-002가 `insertImageFromDialog` 본체를 모드 인지로 전환하면서 "inline-blob 분기에서는 `mdFilePath`를 사용하지 않게" 만들어 놓고도, 호출부 쪽 게이트는 SPEC-IMG-MODE-002 Non-Goal #6로 명시적으로 손대지 않기로 한 영역이라 비대칭이 남아 있었습니다. 본 SPEC이 해당 Non-Goal을 폐기하고 두 진입점에 동일한 모드 인지 분기를 적용했습니다 — **Inline** 모드에서는 Save-As를 건너뛰고 빈 경로로 `insertImageFromDialog`를 직접 호출, **File** 모드(`./images/` 폴더로 복사 후 상대경로 링크)에서는 기존대로 Save-As 게이트를 유지합니다. 이미 저장된 문서에서는 모드와 무관하게 기존 동작을 유지합니다. 클립보드 붙여넣기 경로는 0.10.1에서 같은 이유로 이미 좁혀진 상태라 이번에 건드리지 않았습니다.
...
### Fixed
- **목록에서 불릿과 글씨가 두 줄로 갈라지던 문제 (SPEC-PREVIEW-011)**: 항목 사이에 빈 줄이 있는 목록(loose list)에서 `•`나 `1.` 같은 마커만 한 줄을 차지하고 항목 글씨는 그 아래 줄로 밀려났습니다. 같은 문서 안의 빈 줄 없는 목록은 멀쩡해서 원인을 짚기 어려웠습니다. 마크다운 표준상 항목 사이에 빈 줄이 하나라도 있으면 목록 전체의 각 항목 내용이 문단(`<p>`)으로 감싸지는데, 프리뷰 CSS가 마커를 항목 안쪽(`list-style-position: inside`)에 두고 있어 블록 요소인 문단이 마커와 같은 줄에 놓이지 못하고 새 줄에서 시작한 것이 원인입니다. 마커를 바깥(`outside`)으로 옮기고 마커 자리를 목록의 좌측 여백으로 확보했습니다. 함께 있던 간격 결함도 수정 — 빈 줄 있는 목록은 항목 간격이 빈 줄 없는 목록보다 눈에 띄게 넓었는데, 목록의 항목 간격과 문단 하단 여백이 겹쳐 더해진 탓이라 항목의 마지막 문단에만 하단 여백을 걷어냈습니다(여러 문단으로 된 항목의 문단 구분은 그대로 유지됩니다). 화면 프리뷰와 HTML/PDF 내보내기가 서로 다른 스타일시트를 쓰고 있어 양쪽에 같은 규칙을 적용했습니다. 이 수정으로 **모든 문서의 모든 목록 들여쓰기가 소폭 바뀝니다** — 마커가 차지하던 자리가 좌측 여백으로 이동한 결과이며, GitHub·VS Code 등 표준 렌더러와 같은 모양이 됩니다.
- **✨ 다이어그램 서브메뉴가 클릭에 반응하지 않던 문제 (SPEC-AI-011)**: ✨ 선택 툴바에서 「🧜 다이어그램으로」를 마우스로 클릭하면 아무 반응이 없었습니다 — 서브메뉴를 열려면 항목에 마우스를 올린 뒤 포인터를 서브메뉴 쪽으로 옮기는 수밖에 없었고, 클릭 자체는 완전히 먹통이었습니다(키보드 사용자는 영향 없음). 원인은 트리거가 "열기"는 `mouseenter`에, "토글"은 `click`에 각각 바인딩돼 있었던 것 — 실제 마우스 클릭은 항상 `mouseenter`가 먼저 발생하므로, 클릭할 때마다 서브메뉴가 열렸다가 곧바로 닫혔습니다. 키보드 Enter/Space는 앞선 `mouseenter` 없이 `click`만 단독으로 발생시켜서 키보드 경로는 멀쩡했고, 기존 유닛 테스트도 같은 이유로 계속 green이었습니다(테스트가 실제 포인터 제스처 대신 단순 `click`만 dispatch해 jsdom에서는 이 버그가 애초에 재현되지 않았습니다 — 검증된 적이 없었을 뿐, 놓친 게 아니었습니다). 이제 트리거 클릭은 열기 전용이자 멱등적으로 동작합니다 — 닫힌 서브메뉴를 클릭하면 열리고, 이미 열린 서브메뉴를 다시 클릭해도 열린 채로 유지됩니다(호버로 여는 동작은 그대로). 같은 작업에서 서브메뉴 내부 키보드 탐색도 새로 추가했습니다 — ArrowDown/ArrowUp으로 8개 항목 사이를 순환 이동, Enter/Space로 정확히 한 번 선택, Escape로 서브메뉴를 닫고 포커스를 트리거로 되돌립니다. 서브메뉴 컨테이너와 항목에는 `role="menu"`/`role="menuitem"`이 부여됐고, 포인터를 다른 프리셋 항목으로 옮기면 서브메뉴가 닫힙니다. AI로 전송되는 페이로드는 그대로입니다(「자동」은 다이어그램 형태를 지정하지 않고, 개별 항목은 각자의 형태를 지정 — 결과는 변하지 않습니다).
