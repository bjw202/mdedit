...
### AC-R-4: per-path 크기 획득 (REQ-R-003)
...
And IPC가 실패하면 시스템은 안전 쪽(사용자 모드 존중)으로 폴백하며 삽입을 중단하지 않는다
```
...
| 다이얼로그 경로 `readFileSize` IPC 실패 | 안전 폴백 — 사용자 모드 존중 (inline-blob → data URI). 삽입 중단 없음 (REQ-R-003) | UT-R-003b 변형 |
| 대형 이미지 붙여넣기 + 미저장 + 사용자가 Save-As 취소 | no-op — 삽입 수행 안 함. 확보된 File 객체 폐기 | UT-U-001 변형 |
...
| 대형 이미지 >10MB가 file-save로 라우팅 | Rust 10MB 검증에서 거부. 사용자 가시적 에러는 Non-Goal #6 (OD-2 관련) | UT-N-002 |
| 매우 빠른 연속 대형 이미지 붙여넣기 (멀티 클립보드) | 첫 이미지 Save-As 확보 후 세션 재사용 여부는 OD-2. 본 SPEC 1차 구현은 이미지마다 게이트 | (수동 스모크 only) |
...
- **Playwright must-pass**: PT-MODE-003-001 (클립보드 대형 이미지 + 미저장 → Save-As 안전), PT-MODE-003-002 (드롭 대형 이미지 + 미저장 → 기존 게이트 동작). 로컬 실행 필수 ([feedback-jsdom-pointer-blindspot]).
...
| REQ-R-003b (readFileSize IPC) | UT-R-003b | — | — | — |
...
- [ ] **RED**: UT-R-001b/d/f, UT-U-001, UT-R-BOUNDARY가 실패 상태로 존재
- [ ] **RED**: 기존 UT-7/8/11이 `readFileSize` 소형 mock으로 여전히 green (회귀 가드 확인)
- [ ] **GREEN**: `IMAGE_INLINE_THRESHOLD` 상수가 `previewLimits.ts`에 추가됨 (OD-1 값)
- [ ] **GREEN**: `resolveImageRoute` helper가 추가됨
- [ ] **GREEN**: `insertImageFile`(붙여넣기)에 per-image 라우팅 + 지연 Save-As 적용됨
- [ ] **GREEN**: `insertImageFromDialog`(다이얼로그)에 per-image 라우팅 + `readFileSize` 조회 + 지연 Save-As 적용됨
...
- [ ] **Playwright must-pass**: PT-MODE-003-001 (클립보드 대형+미저장), PT-MODE-003-002 (드롭 대형+미저장) 로컬 통과
- [ ] **수동 스모크**: AC-R-1, AC-R-2, AC-U-1, AC-U-2, AC-U-3 시나리오 실기기 확인
...
- [ ] **Open Decisions**: OD-1 (임계값), OD-2 (지연 Save-As UX), OD-3 (텔레메트리)가 run phase 개시 전 사용자에 의해 명시적으로 해결됨
...
| AC-R-4 | REQ-R-003 | UT-R-003a/b | Unit |
