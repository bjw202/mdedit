...
- [BUG FIX 2026-07-17] 실기기 다이어그램 결함 체인 해소: (1) 재요청 구독 유실 — 브리지가 원 requestId 고정+첫 done에서 해제되어 자동 재요청/↻ 재요청 결과가 카드에 영영 미반영(streaming 고정) → boundRequestId 추적으로 수정, (2) fireReRequest 원문(selection/context) 유실 → 원 args 스프레드, (3) 모델의 ```mermaid 펜스 → stripMermaidFence + 프롬프트에 펜스 금지 지시, (4) 글로우 조기 소멸은 CSS 재현 불가(고착 상태의 부수 체감) — 표준 mask 셔트핸드 방어 추가. vitest 875 / cargo 214 / e2e 7
- [BUG FIX 2026-07-17] 다이어그램 삽입 펜스 유실(BUG-5, 펜스 strip 수정의 후속 결함): 문서 삽입 시 벗긴 코드가 그대로 들어가 프리뷰가 일반 텍스트로 렌더 → ensureMermaidFence를 삽입 경계(diagram 프리셋 한정)에 적용, 카드 상태·검증·미니 프리뷰는 벗긴 코드 유지. vitest 896 / e2e 7
- [BUG FIX 2026-07-17] 목록 폴백 무한 루프(BUG-6): '✓ 목록으로' 재요청 결과가 여전히 다이어그램 검증 분기로 들어가 재실패·재폴백 반복 → enterListFallback 모드 전환(완료를 일반 제안으로 처리 + 삽입 시 펜스 미포장). 스켈레톤 공백 판정(BUG-7): 첫 청크가 개행뿐이어도 스켈레톤·글로우 shimmer가 꺼지던 것을 trim 기준으로 수정 — "출력 직전까지 유지" 충족. vitest 901 / e2e 7
- [GAP FIX 2026-07-17] REQ-AI-028/029 구현 누락 보완: 3초 멈춤 힌트 버튼(빈 헤딩 "이 섹션 채우기"/문서 끝 "이어쓰기", 토큰 0), 문서 끝 이어쓰기 트리거(presetKind 'continue' + Rust Continue 프롬프트 변형), 고스트 컨트롤 버튼([■ 중지]/[✓ 넣기]/[✕ 지우기]). Tab 정책은 REQ-AI-031대로 유지(사용자 확정). vitest 894 / cargo 221 / tsc 0 / e2e 7. 자유 위치 이어쓰기(시나리오 E 전체)는 M2 유지
- [UX FIX 2026-07-17] 하단 트리거 가시성(BUG-8/9): 프리셋 메뉴 뷰포트 하단 공간 부족 시 위로 플립(decideMenuFlipDirection), 카드 mount/phase 전환 시 scrollIntoView(nearest, 청크마다 금지), 고스트 트리거·done 전환 시 1회 스크롤. vitest 913 / e2e 8 (긴 문서 하단 여정 추가)
