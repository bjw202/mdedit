...
> 자동 생성 요약. 정본은 `spec.md` (v1.2.0). 연구 산출물: `docs/design/html-preview.md`.

- **id**: SPEC-PREVIEW-004 | **version**: 1.2.0 | **status**: draft | **priority**: P2 | **author**: jw | **issue**: 0
- **deps**: SPEC-FS-001, SPEC-PREVIEW-001, SPEC-UI-001, SPEC-UI-003
...
- **REQ-PREVIEW004-003** (Ubiquitous + Event-driven + Optional): `tauri.conf.json`에 `assetProtocol: { enable: true, scope: [] }`(빈 정적 scope = 보안 기본값). 폴더 열기·전환 시 Rust 백엔드가 `allow_directory(<정규화 경로>, true)`로 런타임 등록. 세션 누적. `forbid_*` 미사용(영구적이라 재오픈 차단). 경로 정규화는 SPEC-FS-001과 일관. **보안 불변식**: scope는 "사용자가 명시적으로 연 폴더만 포함(bounded)" — "재시작 시 빈 상태"가 아님. SPEC-UI-003 시작 시 자동 복원이 직전 폴더를 즉시 다시 등록하지만, 그 폴더는 사용자가 이전에 명시적으로 연 폴더이므로 불변식 유지.
...
보안 시나리오 3(앱 권한 탈취 차단)·4(`A/**` 미매칭 경로 탈출 차단 + 심링크 우회 차단)·5(미등록 절대 경로 차단)·5-A(재시작 후 scope가 bounded: 자동 복원 폴더 파일 200 + 폴더 밖 `/etc/passwd` 403), 회귀 시나리오 7(기존 마크다운·수식·다이어그램 정상).
