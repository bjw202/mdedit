...
- **세션 누적(session-cumulative)**: 런타임 scope는 사용자가 이번 세션에서 명시적으로 연 폴더만 누적하며, 앱 재시작 시 빈 상태로 초기화된다.
...
- The system **shall** 런타임 scope를 세션 누적 방식으로 관리하여 사용자가 명시적으로 연 폴더만 허용하며, 앱 재시작 시 빈 상태로 초기화한다. `forbid_*` 계열은 영구적이고 재오픈을 막으므로 사용하지 않는다.
