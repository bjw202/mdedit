...
실행: `npm run tauri dev` 후 아무 문서에서 텍스트를 선택 → ✨ 클릭 → 프리셋 메뉴 노출.

- [ ] **B-1 포인터 클릭 (핵심 회귀)** — 마우스로 "🧜 다이어그램으로"를 **클릭**한다. 서브메뉴가 열린 채 유지된다(수정 전에는 아무 일도 일어나지 않았다). 같은 항목을 여러 번 연타해도 닫히지 않는다. → REQ-001, 002
