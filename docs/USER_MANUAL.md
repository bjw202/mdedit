...
> Mermaid 렌더링 오류가 발생하면 [Appendix: Mermaid 다이어그램 문법 오류 해결](#appendix-mermaid-%EB%8B%A4%EC%9D%B4%EC%96%B4%EA%B7%B8%EB%9E%A8-%EB%AC%BC%EB%B2%95-%EC%98%A4%EB%A5%98-%ED%95%B4%EA%B2%B0)을 참고하세요.
...
### 예제 1: AI 응답을 메신저로 공유하기

**상황**: ChatGPT나 Gemini에게 물어본 답변을 Slack이나 카카오톡으로 팀원에게 공유하고 싶을 때

**단계별 가이드**:

1. **AI에게 마크다운 형식으로 요청**

   > "이 내용을 마크다운 형식으로 정리해줘. 제목, 목록, 강조를 사용해서"

2. **AI 답변 복사 (중요!)**

   > **⚠️ ChatGPT/Gemini에서는 클립보드 아이콘을 눌러야 합니다!**

   - **ChatGPT**: 응답 하단의 **클립보드 아이콘** 📋 클릭
   - **Gemini**: 응답 하단의 **복사 버튼** 클릭
   - **Claude**: Ctrl+A → Ctrl+C (Claude는 가능합니다)

3. **MdEdit에 붙여넣기**

   - 편집 창에 Ctrl+V로 붙여넣기
   - 오른쪽 미리보기에서 렌더링 결과 확인

4. **내보내기**

   - 상단 메뉴의 \[Export ▼\] → Export as PDF 클릭
   - 파일 이름 지정 후 저장

5. **공유**

   - 생성된 PDF 파일을 메신저에 드래그 앤 드롭
   - 또는 HTML로 내보내서 브라우저에서 바로 공유

**팁**: PDF는 모든 기기에서 동일하게 보이므로 공유용으로 가장 좋습니다.

---

### 예제 2: 이메일로 보고서 보내기

**상황**: AI가 작성한 보고서를 이메일 첨부 파일로 보내야 할 때

**단계별 가이드**:

1. AI에게 "보고서 형식으로 마크다운으로 작성해줘"라고 요청
2. 답변을 복사 (ChatGPT/Gemini: 클립보드 아이콘 📋 클릭)
3. MdEdit에 붙여넣기
3. 미리보기에서 확인 후 필요한 부분 수정
4. \[Export ▼\] → Export as DOCX 선택 (Word 형식)
5. 이메일에 첨부하여 전송

**팁**: DOCX로 내보내면 받는 사람이 Word에서 수정하기 좋습니다.

---

### 예제 3: 링크가 포함된 문서 처리

**새로운 기능 v0.3.0**: 미리보기 패널의 링크를 클릭하면 바로 브라우저에서 열립니다!

**사용 방법**:

- AI가 참조 링크를 포함한 답변을 준 경우
- MdEdit 미리보기 패널에서 링크를 클릭
- 시스템 기본 브라우저(Safari, Chrome 등)에서 자동으로 열림
...
- [API 레퍼런스](https://api.example.com)
```

미리보기에서 이 링크들이 클릭하면 각각 브라우저의 새 탭에서 열립니다.

---

### 예제 4: 스크린샷으로 AI 답변 보강하기

**상황**: AI가 설명한 내용을 스크린샷으로 찍어서 문서에 함께 포함하고 싶을 때

**언제 유용한가요?**
- AI가 작성한 코드 실행 결과를 보여줄 때
- AI가 설명한 UI/디자인을 시각적으로 보여줄 때
- AI가 생성한 다이어그램이나 차트를 포함할 때
- 에러 메시지나 로그를 문서화할 때

**단계별 가이드**:

1. **AI에게 답변 요청**
   > "이 코드를 설명해줘. 실행 결과도 같이 보여줘."

2. **AI 답변을 MdEdit에 붙여넣기**
   - ChatGPT/Gemini: 클립보드 아이콘 📋 클릭
   - MdEdit 편집 창에 Ctrl+V로 붙여넣기

3. **스크린샷 찍기**
   - **macOS**: Cmd+Shift+4 (영역 선택) 또는 Cmd+Shift+3 (전체 화면)
   - **Windows**: Win+Shift+S (영역 선택)
   - 이미지가 자동으로 클립보드에 복사됨

4. **이미지를 문서에 삽입**
   - MdEdit 편집 창에서 이미지를 넣을 위치 클릭
   - Ctrl+V로 붙여넣기
   - 미리보기에서 이미지가 렌더링되는 것 확인

5. **완성**
   - 텍스트 + 스크린샷이 포함된 완성된 문서
   - PDF나 HTML로 내보내서 공유

**팁**:
- 스크린샷은 Ctrl+V 한 번으로 바로 삽입됩니다 (파일 선택 불필요)
- 여러 개의 스크린샷을 연속으로 붙여넣을 수 있습니다
- 미리보기에서 실제 이미지처럼 보이고, 내보내기 시에도 포함됩니다

**예시 워크플로우**:

```
AI 답변 받기           스크린샷 캡처          MdEdit에 붙여넣기
(코드 설명)    →    (결과 화면)      →   [텍스트 + 이미지]
                                                    ↓
                                               PDF로 공유
```
...
## Appendix: Mermaid 다이어그램 문법 오류 해결

\[기존 "7. Mermaid 다이어그램 트러블슈팅" 섹션을 여기로 이동\]
...
클립보드 아이콘 클릭 📋                 Ctrl+V 로 붙여넣기          렌더링 결과 확인
(ChatGPT/Gemini) 또는 Ctrl+A→C (Claude)
```
...
> **⚠️ 주의**: ChatGPT, Gemini 등에서는 **Ctrl+A로 전체 선택하면 안 됩니다!**

**올바른 복사 방법**:

1. **ChatGPT**: 응답 하단의 **클립보드 아이콘** 📋을 클릭
2. **Gemini**: 응답 하단의 **복사 버튼**을 클릭
3. **Claude**: Ctrl+A로 전체 선택 후 Ctrl+C

**왜 클립보드 아이콘을 눌러야 할까요?**

ChatGPT와 Gemini는 웹페이지에서 텍스트를 선택하면 **단순 텍스트**로 복사됩니다. 하지만 **클립보드 아이콘**을 클릭하면 **마크다운 형식**(`#`, `**`, `-` 등)이 포함된 상태로 복사됩니다.

```
❌ Ctrl+A → Ctrl+C: 제목 목록 강조 모두 사라짐
✅ 클립보드 아이콘 클릭: 마크다운 형식 그대로 복사됨
```
...
### 4-3. 스크린샷 붙여넣기 & 이미지 삭제

스크린샷을 클립보드에 복사한 뒤 편집 창에 **Ctrl+V** 로 붙여넣으면 이미지가 삽입됩니다.

**편집 창에서 이미지 보이는 방식:**

편집 창에서는 긴 base64 코드 대신 **썸네일 위젯**으로 표시됩니다.
...
썸네일 위젯을 클릭하여 커서를 이미지 옆에 위치시킨 후, **Delete** 또는 **Backspace** 키를 누르면 이미지 전체가 한 번에 삭제됩니다.

> **팁**: 이미지는 원자적 단위로 처리되므로, 한 번의 키 입력으로 전체 이미지 데이터가 삭제됩니다.
...
#### 문제 1: 백틱 3개 + mermaid 뒤에 공백이나 글자가 있는 경우

**잘못된 예시:**

백틱 기호 3개 뒤에 공백이 있으면 안 됩니다.
...
**텍스트로 표현하면**: 백틱 3개 + 줄바꿈 + mermaid → ✅ 렌더링 성공

**해결 방법**: 백틱 3개 다음 줄바꿈 후 mermaid 키워드가 와야 합니다.
...
이렇게 특수문자를 포함한 노드는 따옴표로 감싸지 않으면 렌더링이 실패합니다.
...
\[Step 1\] \`\`\`mermaid 줄 확인 - 줄 끝에 공백이나 다른 문자가 없는지 확인 - 대소문자가 정확한지 확인 (mermaid, flowchart 등은 소문자)
