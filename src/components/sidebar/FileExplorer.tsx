...
export function FileExplorer(): JSX.Element {
  ...
  if (!watchedPath) {
    return (
      <div className="md-sidebar h-full flex flex-col items-center justify-center gap-3 p-4">
        <svg
  ...
  if (isLoading) {
    return (
      <div className="md-sidebar h-full flex items-center justify-center text-xs" style={{ color: 'var(--md-text-faint)' }}>
        Loading...
      </div>
    );
  }
      ...
      <div className="md-tree">
        ...
        {canGoUp && (
          <button
            ...
            onClick={handleGoUp}
            title={parentPath ?? ''}
            className="md-tree-row w-full"
          >
            <ChevronRightIcon width={13} height={13} className="chev" style={{ visibility: 'hidden' }} />
            <FolderIcon width={15} height={15} className="ficon" style={{ color: 'var(--md-accent)' }} />
